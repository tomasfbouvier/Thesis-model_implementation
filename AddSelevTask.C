AliAnalysisSelevTask* AddSelevTask(TString name = "FlowTask", TString weightsFile = "", TString eventsSelectedFile = ""  ,const char* suffix = "")
{
    ...
    Bool_t useEventSelector = kFALSE;
    if(!eventsSelectedFile.IsNull()) useEventSelector = kTRUE;

    AliAnalysisSelevTask* task = new AliAnalysisSelevTask(taskName.Data(), useWeights, useEventSelector);
    if(!task) return 0x0;
    ...
    if(useEventSelector) {
      ...
      AliAnalysisDataContainer* eventSelection = (AliAnalysisDataContainer*) taskContainers->FindObject("eventsSelected");
      if(!eventSelection) {
        if(eventsSelectedFile.Contains("alien://")) { gGrid->Connect("alien://"); }

        printf("input file name: %s \n", eventsSelectedFile.Data());
        
        TFile* events_file = TFile::Open(eventsSelectedFile.Data(),"READ");
        if(!events_file) { printf("Input file with events not found!\n"); return NULL; }

        TTree* events_list = (TTree*) events_file->Get("tree1");
        if(!events_list) { printf("E-AddTaskUniFlow: Input list with events not found!\n"); events_file->ls(); return NULL; }
        
        AliAnalysisDataContainer* cInputevents = mgr->CreateContainer("inputevents",TTree::Class(), AliAnalysisManager::kInputContainer);
        cInputevents->SetData(events_list);
        
        mgr->ConnectInput(task,2,cInputevents);
      }
      else {
        mgr->ConnectInput(task,2,eventSelection);
      }
    }
