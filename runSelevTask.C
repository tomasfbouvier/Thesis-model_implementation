...
void runSelevTask()
{
...
#if !defined (__CINT__) || defined (__CLING__)
    gInterpreter->LoadMacro("AliAnalysisSelevTask.cxx++g");
    AliAnalysisSelevTask *task = reinterpret_cast<AliAnalysisSelevTask*>(gInterpreter->ExecuteMacro("AddSelevTask.C(\"FlowTask\", \"~/Desktop/Thesis-training-/Weights/weight.root\", \"~/Desktop/Thesis-model_implementation/selection.root\")"));
#else
    gROOT->LoadMacro("AliAnalysisSelevTask.cxx++g");
    gROOT->LoadMacro("AddSelevTaskTask.C");
    AliAnalysisSelevTask *task = AddSelevTaskTask();
#endif
    ...
    if(local) {
        ...
        alienHandler->SetAdditionalLibs("AliAnalysisSelevTask.cxx AliAnalysisSelevTask.h");
        // alienHandler->SetAdditionalLibs("PWGCFFLOWGF.par libPWGEMCALbase.so");
        alienHandler->SetAnalysisSource("AliAnalysisSelevTask.cxx");
