...
void AliAnalysisGetWeights::UserCreateOutputObjects()
{
    ...
    tree->Branch("momentum",&point,"px:py:pz");
...
void AliAnalysisGetWeights::UserExec(Option_t *)
{
    ...
    for(Int_t i(0); i < iTracks; i++) {
        ...
        eventn = (Int_t)fAOD->GetHeader()->GetTimeStamp();
        point.px= track->Pt()*cos(track->Phi());
        point.py= track->Pt()*sin(track->Phi());
        point.pz=  track->Pt()*sinh(track->Eta());
        tree->Fill();
