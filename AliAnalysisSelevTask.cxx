...
#include "AliAnalysisSelevTask.h"

class AliAnalysisSelevTask;

using namespace std;

ClassImp(AliAnalysisSelevTask)

AliAnalysisSelevTask::AliAnalysisSelevTask() : AliAnalysisTaskSE(),
    fAOD(0),
...
AliAnalysisSelevTask::AliAnalysisSelevTask(const char* name, Bool_t bUseWeights, Bool_t bUseEventSelector) : AliAnalysisTaskSE(name),
    fAOD(0),
    fTrigger(AliVEvent::kINT7),
    fIsPP(kFALSE),
    fEventRejectAddPileUp(kFALSE),
    fUseWeights(bUseWeights),
    fUseEventSelector(bUseEventSelector),
    fFilterBit(96),
    fCentBinNum(10),
    fCentMin(0),
    fCentMax(100),
    ...
    fTPCchi2perClusterMax(4.)
{
    ...
    if(bUseEventSelector) { DefineInput(2, TTree::Class()); }
    DefineOutput(1, TList::Class());
}
//_____________________________________________________________________________
AliAnalysisSelevTask::~AliAnalysisSelevTask()
{
    if(fOutputList) delete fOutputList;
}
//_____________________________________________________________________________
void AliAnalysisSelevTask::UserCreateOutputObjects()
{
    ...
    if(fUseEventSelector) fEventSelection = (TTree*) GetInputData(2);
    
    PostData(1, fOutputList);
}
//_____________________________________________________________________________
void AliAnalysisSelevTask::UserExec(Option_t *)
{
    ...
    if(fUseEventSelector && !IsEventSelectionLoaded() ){
        return;}
...
//_____________________________________________________________________________
void AliAnalysisSelevTask::Terminate(Option_t *)
{}
//_____________________________________________________________________________
Bool_t AliAnalysisSelevTask::IsEventSelected()
{
    ...
    if(fUseEventSelector && !CheckEvent()){return kFALSE;}
    fhEventCounter->Fill("ML selection OK",1);

    return kTRUE;
}
//_____________________________________________________________________________
Bool_t AliAnalysisSelevTask::IsTrackSelected(const AliAODTrack* track) const
{
...
//_____________________________________________________________________________
Bool_t AliAnalysisSelevTask::IsEventRejectedAddPileUp() const
{
...
void AliAnalysisSelevTask::update_Q(vector<Double_t> &Q2, Double_t &Total_weight_A, Double_t &Total_weight_B, Int_t n, Double_t Phi, Double_t Eta, Double_t weight){
...
Double_t AliAnalysisSelevTask::C2(const vector<Double_t> Q2){
...
Bool_t AliAnalysisSelevTask::AreWeightsLoaded()
{
...
Bool_t AliAnalysisSelevTask::IsEventSelectionLoaded()
{
  if(!fEventSelection) {AliError("Weights input list not loaded"); return kFALSE; }
     //! fhWeight = (TH1D*)fInputWeights->FindObject("hWeight");
    //!if(!fhWeight) {AliError("Weights not loaded"); return kFALSE; }
  return kTRUE;
}
...
//_____________________________________________________________________________
Double_t AliAnalysisSelevTask::GetWeight(const Double_t dPhi)
{
...
Bool_t AliAnalysisSelevTask::CheckEvent(){
    TTreeReader myReader(fEventSelection);
    TTreeReaderValue<Double_t> eventns(myReader, "branch1");
    
    while (myReader.Next()){
        if(*eventns==fAOD->GetHeader()->GetTimeStamp()) {
            return kTRUE;
        }
//!cout<<*eventns<<"\t"<<(Int_t)fAOD->GetHeader()->GetTimeStamp()<<"\n";
    }
    return kFALSE;
}
